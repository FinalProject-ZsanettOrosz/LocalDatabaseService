...
import introsde.assignment.soap.model.AchivedGoal;
import introsde.assignment.soap.model.CustomMeasureDefinition;
import introsde.assignment.soap.model.Goal;
...
public class PeopleImpl implements People {
	...
	public Goal readGoal(int id) {
		...
		if (p != null) {
			System.out.println("---> Found goal by id = " + id + " => "
					+ p.getGoalValue());
		} else {
	...
	// achivement list
	@Override
	public List<AchivedGoal> readAchivedGoalList(int personId) {
		Person p = Person.getPersonById(personId);
		if (p == null)
			return null;
		return p.getAchivedGoals();
	}

	// achivement by id
	@Override
	public AchivedGoal readAchivedGoal(int id) {
		System.out.println("---> Reading achived goal by id = " + id);
		AchivedGoal p = AchivedGoal.getAchivedGoalById(id);
		if (p != null) {
			System.out.println("---> Found goal by id = " + id + " => "
					+ p.getAchivementDate());
		} else {
	...
	@Override
	public Person saveAchivedGoal(int id, AchivedGoal achivedGoal) {

		Person person = Person.getPersonById(id);

		// if the goal is present here today then no saving
		List<AchivedGoal> all = person.getAchivedGoals();
		boolean alreadyMetTheGoal = false;
		for (int i = 0; i < all.size(); i++) {
			if (all.get(i).getAchivementDate()
					.equals(achivedGoal.getAchivementDate())
					&& all.get(i).getAchivedGoal().getIdGoal() == achivedGoal
							.getAchivedGoal().getIdGoal()) {
				alreadyMetTheGoal = true;
				System.out.println(all.get(i).getAchivementDate());
				System.out.println(achivedGoal.getAchivementDate());
			}
		}
		if (alreadyMetTheGoal == false) {
			achivedGoal.setPerson(person);
			person.setAchivedGoals(achivedGoal);
			AchivedGoal.saveAchivedGoal(achivedGoal);
			Person.updatePerson(person);
		} else {
			System.out.println("LD already saved this achivement");
		}

		// all.get(i).getAchivedGoal().getIdGoal() ==
		// achivedGoal.getAchivedGoal().getIdGoal()
		return person;
	}
	...
	public Person updatePerson(Person person) {
		...
		if (existing == null) {
			...
			if (person.getName() != null
					&& !fromDB.getName().equals(person.getName())) {
				fromDB.setName(person.getName());
			}
			if (person.getLastname() != null
					&& !fromDB.getLastname().equals(person.getLastname())) {
				fromDB.setLastname(person.getLastname());
			}
			if (person.getBirthdate() != null
					&& fromDB.getBirthdate() == person.getBirthdate()) {
				fromDB.setBirthdate(person.getBirthdate());
			}
			if (person.getEmail() != null
					&& !fromDB.getEmail().equals(person.getEmail())) {
				fromDB.setEmail(person.getEmail());
			}
			if (person.getUsername() != null
					&& !fromDB.getUsername().equals(person.getUsername())) {
	...
	// M #6
	@Override
	public List<HealthMeasureHistory> readPersonHistory(int id,
			String measureType) {
	...
	public Person savePersonMeasure(int id, LifeStatus newLifeStatus) {
		...
		if (oldLifeStatus == null) {
			...
			HealthMeasureHistory witOldLFS = new HealthMeasureHistory(
					oldLifeStatus);
	...
	// M #10
	@Override
	public HealthMeasureHistory updatePersonMeasure(int id,
			HealthMeasureHistory newHistory) {
		Person person = Person.getPersonById(id);
		List<HealthMeasureHistory> personHis = person.getMeasureHistories();
		HealthMeasureHistory his = null;
		for (int i = 0; i < personHis.size(); i++) {
			if (personHis.get(i).getIdMeasureHistory() == newHistory
					.getIdMeasureHistory()) {
				his = personHis.get(i);
			}
		}
